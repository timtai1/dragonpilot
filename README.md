...
### 3. 🍃 縱向加速與煞車舒緩優化 (Gentle Acceleration & Early Smooth Decel)
* **溫和加速選單 (Gentle Acceleration)**：
  * 於 dp 縱向控制 (Longitudinal) 選單新增「溫和加速」設定。
  * 提供四段起步最大加速度上限：`1.0`、`1.2`、`1.4`、`1.6` m/s²（預設為 **`1.2` m/s²**）。
  * 解決原廠 1.6 m/s² 起步推力過猛、油門突然深踩被「猛踹一腳」的不適感，享受線性且溫和舒適的起步。
* **提前 2 倍距離平順減速 (Early Smooth Braking)**：
  * 解決行駛中遇前方等紅燈或慢速前車時「接近才急煞」的痛點。
  * 改以原本 2 倍距離處提前開始平順收油與減速，將煞車力道減半至舒適的 ~1.25 m/s²（原廠為重踩等級的 2.5 m/s²），紅燈煞停更加優雅絲滑。
