...
## **🔥 本分支更新與客製功能 (Custom Updates)**

本分支（`0.11.1`）針對 Comma 3 / 3X 與實際用車體驗進行了以下優化與功能擴充：

* **🎡 方向盤位置手動設定 (Wheel on Left or Right?)**
  * **自訂設定項目**：於 dp 設定選單末端新增「方向盤在左或在右？（Wheel on Left or Right?）」開關，支援 `Left`（左駕，如台灣、美國）與 `Right`（右駕，如日本、香港、英國）手動切換，預設為 `Left`。
  * **移除動態誤判**：徹底拔除原版 `policy.py` 透過駕駛鏡頭持續動態猜測左右駕的統計過濾演算法，避免夜間、逆光或坐姿變換導致判定跳動，並改為直接讀取手動設定值，跨國駕駛亦不會自動變更。

* **🇹🇼 繁體中文在地化與多國語言支援**
  * **翻譯模組重構**：重寫 `multilang.py`，支援直接解析與載入 `dragonpilot_{lang}.po` 語言檔。
  * **繁體中文 100% 覆蓋**：完整補齊 `dragonpilot_zh-CHT.po`，解決在繁中系統下 dp 選單依然顯示英文的問題。

* **🔤 點陣字型圖集烘焙 (解決問號 `?` 缺字問題)**
  * **字型工具升級**：更新 `selfdrive/assets/fonts/process.py`，自動抽取所有翻譯檔中的中文字元。
  * **CJK Bitmap Font Atlas 重新烘焙**：在裝置端利用 Raylib 重新烘焙高解析度點陣字型圖集（`OpFont-*.fnt` / `OpFont-*.png`），徹底解決介面上繁中字元渲染為 `?` 的缺字現象。

* **⚡ UI 穩定性優化與快速熱重載**
  * **修復崩潰問題**：解決布林參數轉型時引發的 `ValueError` 導致點擊 dp 選單閃退重啟之錯誤。
  * **快速熱重載機制**：建立無需重新開機（免等 10-15 分鐘）的 Hot Reload 流程，代碼推送後 1 秒內即可無縫重載 UI。
