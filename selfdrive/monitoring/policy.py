...
class DriverMonitoring:
  def __init__(self, rhd_saved=False, settings=None, always_on=False):
    ...
    self.params = Params()
    self.wheel_on_right = self.params.get_bool("dp_dev_is_rhd")
    self.wheel_on_right_last = None
    self.wheel_on_right_default = self.wheel_on_right
  ...
  def _update_states(self, driver_state, cal_rpy, car_speed, op_engaged, standstill, demo_mode=False, steering_angle_deg=0.):
    # dp - read wheel position setting instead of auto detection
    self.wheel_on_right = self.params.get_bool("dp_dev_is_rhd")
